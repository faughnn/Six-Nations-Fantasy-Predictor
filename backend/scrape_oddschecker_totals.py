"""
Standalone script to scrape total points (over/under) odds from Oddschecker.

Auto-discovers Six Nations match URLs and scrapes per-bookmaker over/under odds.
Saves raw JSON to backend/data/oddschecker/ and optionally persists averaged
odds to the database.

Usage:
    python scrape_oddschecker_totals.py                         # headless, all matches
    python scrape_oddschecker_totals.py --headed                # visible browser
    python scrape_oddschecker_totals.py --match france-v-ireland
    python scrape_oddschecker_totals.py --save-db --season 2026 --round 1
"""
...
def print_summary_table(parsed_data):
    """Print the consensus total points line to the console."""
    if not parsed_data:
        print("\nNo totals data found.")
        return

    item = parsed_data[0]
    over_odds = item.get("over_odds", 0) or 0
    under_odds = item.get("under_odds", 0) or 0
    num_bk = item.get("num_bookmakers", 0)

    print(
        f"\nConsensus total points: {item['line']:.1f}  "
        f"(over: {over_odds:.2f}, under: {under_odds:.2f}, {num_bk} bookmakers)"
    )
...
async def scrape_match(scraper: OddscheckerScraper, url: str, slug: str):
    ...
    logger.info(f"Scraping total points odds: {url}")
    raw_data = await scraper.scrape_match_totals(url)
    parsed_data = scraper.parse(raw_data)

    # Save raw JSON (includes per-bookmaker breakdown)
    scraper.save_raw_json(raw_data, f"{slug}_totals")
...
async def save_to_db(parsed_data, season: int, round_num: int, match_date: date,
    ...
    """Persist total points odds to the database via OddsService."""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Scrape Oddschecker total points (over/under) odds for Six Nations matches",
    )
    parser.add_argument(
    ...
    if args.match:
        slug = args.match
        base_url = f"https://www.oddschecker.com/rugby-union/six-nations/{slug}/total-points"
        matches_to_scrape = [{
            "slug": slug,
        ...
        # Append /total-points to each discovered URL
        for m in matches_to_scrape:
            base = m["url"].rstrip("/")
            if "/winner" in base or "/anytime" in base or "/total" in base:
                base = base.rsplit("/", 1)[0]
            m["url"] = f"{base}/total-points"
