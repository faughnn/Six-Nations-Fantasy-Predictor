...
function getValueTierClass(pointsPerStar: number | null): string {
  ...
  if (pointsPerStar >= 15) return 'bg-green-50/60';
  if (pointsPerStar >= 10) return 'bg-green-50/40';
  if (pointsPerStar >= 7) return 'bg-amber-50/50';
  return '';
}
        ...
        <tbody className="divide-y divide-stone-200">
          {sortedData.map((player) => (
            ...
            >
              {/* Fixed columns */}
              <td className={cn(
                'px-3 py-1.5 whitespace-nowrap font-medium text-stone-800 sticky left-0 z-10',
                getValueTierClass(player.points_per_star) || 'bg-[#faf8f4]'
              )}>
                {player.name}
              </td>
